...
def _sev_color(severity: str, active: bool) -> tuple[float, float, float]:
    if active:
...
def highlight_pdf(pdf_bytes: bytes, items: list[dict], active_key: str = "") -> tuple[bytes, int | None]:
    ...
    try:
        ...
        for number, item in enumerate(items, start=1):
            ...
            sev = str(item.get("severity", "yellow"))
            ...
            for phrase in _search_frags(text):
                ...
                for page_index, page in enumerate(doc):
                    ...
                    if not matches:
                        continue
                    popup_text = (
                        f"[#{number}] Suggested rewrite:\n{item.get('rewritten', '')}\n\n"
                        f"Why: {item.get('reasoning', '')}"
                    )
                    for quad in matches[:3]:
                        annot = page.add_highlight_annot(quad)
                        annot.set_colors(stroke=_sev_color(sev, is_active))
